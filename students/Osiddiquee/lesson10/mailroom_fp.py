...
'''
Functional stuff is up top
'''
def map_function():
    (keys, donor) = get_donor_db().items()
    (name, amounts) = donor
    factor = input('What factor would you like?\n>')
    donor_temp = zip(name, list(map(lambda x: x * factor, amounts)))
    donor = donor_temp
    return dict(zip(keys, donor))
...
def filter_function():
    ...
    max = input('Max?\n>')
    min = input('Min?\n>')
    donor_temp = zip(name, filter(lamda x: x < max and x > min,
                list(map(lambda x: x * factor, amounts))))
    donor = donor_temp
    return dict(zip(keys, donor))


def reduce_function():
    donors = get_donor_db().items()
    name = input('What is the name?'\n)
    print('Double contirbutions under 100:')
    sum(list(map(lamda x: x * 2, list(filter(lambda x: x < 100, 
                                  donors[name.lower()][1])))) +
                              list(filter(lambda x: x > 100,
                                   donors[name.lower()][1])))))

    print('Triple contirbutions over 50:')
    sum(list(map(lamda x: x * 3, list(filter(lambda x: x > 50,
                                  donors[name.lower()][1])))) +
...
def functional_excerise():
    selection_dict = {"1": map_function,
                      "2": filter_function,
                      "3": reduce_function,
                      "4": quit}

    while True:
        selection = main_menu_functional()
        try:
