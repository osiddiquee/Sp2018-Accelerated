...
class DonorDB:
    ...
    def add_donor(self, donor):
        self._donors[donor.name] = donor.donations
...
def find_donor(name):
    ...
    """
    return donor_db.database_dict.get(name)

def add_donor(donor):
    """
    Add a new donor to the donor db

    :param: the donor object
...
def send_thank_you():
    ...
    if donor is None:
        ...
        donor_db.database_dict[name].append(amount)
...
def generate_donor_report():
    ...
    report_rows = []
    for (name, gifts) in donor_db.database_dict.items():
...
def save_letters_to_disk():
    ...
    """
    for name, donation in donor_db.database_dict.items():
        letter = gen_letter([name, donation])
...
if __name__ == "__main__":
    ...
    d.add_donations(40000)
    d2.add_donations(4000)
